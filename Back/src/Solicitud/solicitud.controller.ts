...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: "found all solicitudes", data: solicitudes });
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    const { fechaSolicitud, estado, idPasajero, idViaje } =
      req.body.sanitizedInput;

    const pasajero = em.getReference(Pasajero, Number.parseInt(idPasajero));
    const viaje = await em.findOneOrFail(Viaje, Number.parseInt(idViaje), {
    ...
    if (solicitudExistente) {
      return res
        .status(409)
        .json({ message: "Solicitud ya existe para este pasajero y viaje" });
    }

    // Validar si hay cupo para hacer la inscripcion
    const solicitudes = viaje.solicitudes.filter(
    ...
    const solicitud = em.create(Solicitud, {
      pasajero,
      viaje,
      fechaSolicitud,
      estado,
    ...
    return res
      .status(201)
      .json({ message: "Solicitud created", data: solicitud });
  } catch (error: any) {
...
async function remove(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: "Solicitud deleted successfully", data: solicitud });
  } catch (error: any) {
    if (error instanceof NotFoundError) {
      return res.status(404).json({ message: "Solicitud not found" });
    }
...
async function findAllByPasajero(req: Request, res: Response) {
  try {
    ...
    res.status(200).json({
      message: "found all solicitudes for pasajero",
      data: solicitudes,
    });
  } catch (error: any) {
    return res.status(500).send({ message: error.message });
